 ...
 * xattr_ops.c
 *
 * Provide generic utility fuctions on xattr operations for ocfs2-test
 *
...
void xattr_setup(void)
{
	...
	for (i = 0; i < xattr_nums; i++)
		xattr_name_list_set[i] = (char *)malloc(XATTR_NAME_MAX_SZ + 1);

	list_sz = (unsigned long)((XATTR_NAME_MAX_SZ + 1) * xattr_nums);

	list = (char *)malloc(list_sz);
	xattr_name_list_get = (char **)malloc(sizeof(char *) * xattr_nums);

	for (i = 0; i < xattr_nums; i++)
		xattr_name_list_get[i] = (char *)malloc(XATTR_NAME_MAX_SZ + 1);
}
