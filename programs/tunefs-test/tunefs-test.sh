...
#
# This script will perform a sanity check on tunefs.ocfs2
#
#
...
Change_Volume_Label()
{
	...
	echo "y"|$TUNEFS_BIN -L $LABEL2 $DEVICE;
	Check_Volume;
...
#
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN -L $LABEL3 $DEVICE;
	Check_Volume;
	SB_LABEL=`$DEBUGFS_BIN -n -R "stats" $DEVICE|$GREP Label| \
		$GAWK '{print \$2; exit}'`;
...
Change_Number_of_Nodes()
{
	...
	echo "y"|$TUNEFS_BIN -N $NNODES2 $DEVICE;
	Check_Volume;
...
#
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN -N $NNODES3 $DEVICE;
	Check_Volume;
	SB_NNODES=`$DEBUGFS_BIN -n -R "stats" $DEVICE|$GREP Slots| \
		 $GAWK '{print \$4; exit}'`;
...
Change_Journal_Size()
{
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN -J size=$JOURNAL2 $DEVICE;
	Check_Volume;
	SB_JSIZE=`$DEBUGFS_BIN -n -R "ls -l //" $DEVICE|$GREP Journal:0001| 
		$GAWK '{print \$6; exit}'`;
...
#
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN -J size=$JOURNAL3 $DEVICE;
	Check_Volume;
	SB_JSIZE=`$DEBUGFS_BIN -n -R "ls -l //" $DEVICE|$GREP Journal:0001| 
		$GAWK '{print \$6; exit}'`;
...
Change_Partition_Size()
{
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN -S $DEVICE $BLKCNT2;
	Check_Volume;
...
#
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN -S $DEVICE $BLKCNT3;
	Check_Volume;
...
Add_Backup_Super()
{
	...
	(( ++NUM_OF_TESTS ))
	echo "y"|$TUNEFS_BIN --backup-super $DEVICE;
	Check_Volume;
